...
export default function TeachingCard({ teaching }) {
  const slug = teaching.slug?.current || teaching._id;
  
  return (
    <Link to={`/teaching/${slug}`} className="block">
      <div className="group relative overflow-hidden rounded-lg shadow-lg transition-transform duration-300 hover:-translate-y-1">
        <img
          ...
          alt={teaching.title}
          className="h-48 w-full object-cover transition-transform duration-300 group-hover:scale-105 opacity-30"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
        <div className="absolute bottom-0 p-4 font-bold">
